...
template<class I, class V>
static __global__ void Set0or1Kernel(const I *index, V *vector, const int n);
...
static __global__ void Set0or1Kernel(const I *index, V *vector, const int n)
{
    ...
    vector[batchIdx * n + elemIdx] = (V)(_index);
    //printf("Set0or1 vector[%d]: %f\n",batchIdx*n+elemIdx, vector[batchIdx*n+elemIdx]);
}
...
static __global__ void OneHotEncodeKernel(const I *index, V *vector, const int n)
{
    ...
    //printf("OnehotEncode vector[%d]: %f\n",batchIdx*n+elemIdx, vector[batchIdx*n+elemIdx]);
}
...
template<class I, class V>
void Set0or1(const I *_index, V *_vector, const unsigned long n,
        const unsigned long batchSize, const cudaStream_t stream)
{
    dim3 dimGrid((n + THREAD_PER_BLOCK - 1) / THREAD_PER_BLOCK, batchSize);
    dim3 dimBlock(THREAD_PER_BLOCK);

    Set0or1Kernel<I, V><<<dimGrid, dimBlock, 0, stream>>>(_index, _vector, n);
}
...
template void Set0or1<int, float>(const int *_index, float *_vector, const unsigned long n,
        const unsigned long batchSize, const cudaStream_t stream);
template void Set0or1<long, float>(const long *_index, float *_vector, const unsigned long n,
        const unsigned long batchSize, const cudaStream_t stream);
template void Set0or1<long long, float>(const long long *_index, float *_vector, const unsigned long n,
        const unsigned long batchSize, const cudaStream_t stream);
template void Set0or1<int, double>(const int *_index, double *_vector, const unsigned long n,
        const unsigned long batchSize, const cudaStream_t stream);
template void Set0or1<long, double>(const long *_index, double *_vector, const unsigned long n,
        const unsigned long batchSize, const cudaStream_t stream);
template void Set0or1<long long, double>(const long long *_index, double *_vector, const unsigned long n,
        const unsigned long batchSize, const cudaStream_t stream);
